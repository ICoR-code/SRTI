...
fprintf('Starting SRTI Wrapper (v2_22_02) for Matlab. Reading settings files... \n');
				...
				while true
                   if (strcmp(subChannels(i).messageName, "RTI_") == 1)
                       for j = 1:length(subChannels(i).varChannel)                 
                           ...
                           if ((isa(setValue,'char') == 1) && setValue(1) == '[')
                               eval("obj." + subChannelObVar + " = " + "jsondecode(setValue)" + ";");
                           elseif ((isa(setValue,'char') == 1) && ~isempty(str2num(setValue)))
