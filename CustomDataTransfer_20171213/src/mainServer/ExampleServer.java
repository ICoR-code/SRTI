...
public class ExampleServer {
		...
		public void handleReceivedMessage(int threadIndex, String message) {
			...
			switch (name){
				...
				case "RTI_PublishTo":
					for (int i = 0; i < threadList.size(); i++) {
						if (threadList.get(i).getIndex() == threadIndex) {
							String newPublishName = Json.createReader(new StringReader(content)).readObject().getString("publishName");
				...
				case "RTI_SubscribeTo":
					for (int i = 0; i < threadList.size(); i++) {
						if (threadList.get(i).getIndex() == threadIndex) {
							String newSubscribeName = Json.createReader(new StringReader(content)).readObject().getString("subscribeName");
	...
	private String buildRTIPublishTo(String publishToName) {
		...
		jsonPublishBuilder.add("publishName", publishToName);
	...
	private String buildRTISubscribeTo(String subscribeToName) {
		...
		jsonSubscribeBuilder.add("subscribeName", subscribeToName);
